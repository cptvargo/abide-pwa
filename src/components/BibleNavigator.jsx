 ...
 * UPDATED: translation prop passed in so Seek shows verses from active translation
 */
...
// ── Uses active translation first, falls back to kjv ─────────────────────────
async function fetchVerseText(ref, translation = "kjv") {
...
async function enrichVerses(verses, translation = "kjv") {
  return Promise.all(
    verses.map(async (v) => {
      const result = await fetchVerseText(v.ref, translation);
      return result
...
export default function BibleNavigator({
  ...
  currentBook,
  currentChapter,
  translation = "KJV", // ── active translation from AppShell
}) {
  ...
  useEffect(() => {
    if (view === "seek") setTimeout(() => seekInputRef.current?.focus(), 120);
  }, [view]);
  ...
  async function handleSeek(queryOverride) {
    ...
    try {
      ...
      // Enrich with active translation
  ...
  const styles = (
    <style>{`
      ...
      @keyframes bn-modal-in  { from{opacity:0;transform:translateY(32px) scale(0.96)} to{opacity:1;transform:translateY(0) scale(1)} }
      @keyframes bn-modal-out { from{opacity:1;transform:translateY(0) scale(1)} to{opacity:0;transform:translateY(24px) scale(0.96)} }
      @keyframes bn-glow-pulse { 0%,100%{opacity:0.4} 50%{opacity:0.75} }
      @keyframes bn-fade-up { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }
      @keyframes bn-shimmer { 0%,100%{opacity:0.3} 50%{opacity:0.65} }

      .bn-backdrop { animation: ${isAnimatingOut ? "bn-backdrop-out 0.32s ease forwards" : "bn-backdrop-in 0.3s ease forwards"}; }
      .bn-modal    { animation: ${isAnimatingOut ? "bn-modal-out 0.28s cubic-bezier(0.55,0,1,0.45) forwards" : "bn-modal-in 0.42s cubic-bezier(0.22,1,0.36,1) forwards"}; }
      .bn-header-content { animation: bn-fade-up 0.4s ease 0.1s both; }
      .bn-glow { animation: bn-glow-pulse 4s ease-in-out infinite; }
      .bn-book { transition: background 0.16s ease, transform 0.14s ease, border-color 0.16s ease; -webkit-tap-highlight-color: transparent; }
      .bn-book:active { transform: scale(0.985) }
      .bn-book-available:hover { background: rgba(var(--accent-rgb,203,178,124),0.13) !important; border-color: rgba(var(--accent-rgb,203,178,124),0.3) !important; }
      .bn-ch { transition: background 0.15s ease, transform 0.13s ease, box-shadow 0.15s ease; -webkit-tap-highlight-color: transparent; }
      ...
      .bn-sticky { position:sticky; top:0; z-index:20; backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); }
      .bn-tab-btn { flex:1; padding:13px 0; background:transparent; border:none; border-bottom:2px solid transparent; font-family:var(--font-ui,system-ui); font-size:11px; letter-spacing:0.12em; text-transform:uppercase; color:rgba(var(--accent-rgb,203,178,124),0.35); cursor:pointer; transition:color 0.18s ease,border-color 0.18s ease; -webkit-tap-highlight-color:transparent; margin-bottom:-1px; }
      .bn-tab-btn.active { color:rgba(var(--accent-rgb,203,178,124),0.9); border-bottom-color:rgba(var(--accent-rgb,203,178,124),0.7); }
      .bn-seek-chip { background:rgba(var(--accent-rgb,203,178,124),0.05); border:1px solid rgba(var(--accent-rgb,203,178,124),0.14); border-radius:20px; padding:8px 16px; font-family:var(--font-body,Georgia,serif); font-style:italic; font-size:14px; color:rgba(var(--accent-rgb,203,178,124),0.65); cursor:pointer; transition:background 0.15s ease,border-color 0.15s ease; -webkit-tap-highlight-color:transparent; }
      .bn-seek-chip:hover { background:rgba(var(--accent-rgb,203,178,124),0.1) !important; border-color:rgba(var(--accent-rgb,203,178,124),0.25) !important; }
      .bn-verse-card { background:rgba(var(--accent-rgb,203,178,124),0.04); border:1px solid rgba(var(--accent-rgb,203,178,124),0.1); border-radius:12px; padding:12px 14px; transition:background 0.15s ease; cursor:default; }
      ...
      <>
        <div
          style={{
            ...
            borderBottom: "1px solid rgba(var(--accent-rgb,203,178,124),0.08)",
            position: "relative",
            zIndex: 1,
            flexShrink: 0,
          }}
        >
          ...
          {seekLoading ? (
            ...
            <div>
              ...
              >
                ✦ &nbsp;Word Study{" "}
                {seekFromCache && <span style={{ opacity: 0.5 }}>· saved</span>}
              </div>
              <h1
                style={{
                  fontFamily: "var(--font-ui,system-ui)",
          ...
          {seekResult && !seekLoading && (
            <div>
              ...
              <SeekSection label="Reflect">
                <p style={{ ...proseStyle, fontStyle: "italic", opacity: 0.6 }}>
